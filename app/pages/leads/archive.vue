    ...
    cell: ({ row }) => {
      ...
      return h("div", { class: "flex items-center gap-1" }, [
        ...
        h(UButton, {
          ...
          size: "sm",
          square: true,
          onClick: () => unarchiveLead(row.original),
        }),
      ]);
    },
  },
...
const loadLeads = async () => {
  ...
  try {
    ...
    const response = await api.get<{ leads: Lead[]; total: number }>(
      apiService.value,
      `/leads/archived?${params.toString()}`,
    );
...
const unarchiveLead = async (lead: Lead) => {
  try {
    await api.patch(apiService.value, `/leads/${lead.id}/unarchive`, {});
    toast.add({
      title: "Muvaffaqiyatli",
      description: "Lead arxivdan chiqarildi",
      color: "success",
    });
    loadLeads();
  } catch (error) {
    ...
    toast.add({
      title: "Xatolik",
      description: "Lead arxivdan chiqarishda xatolik yuz berdi",
      color: "error",
    });
  }
};
